...
<template>
  <NuxtLink class="m-4 relative w-full md:w-1/3 lg:w-1/4 xl:w-1/5" :to="`/store/${id}`"><span v-if="badge"
      ...
      <div>
        ...
        <div class="flex font-medium items-center justify-between mt-4">
          <span class="font-bold">${{ price }}</span><span v-if="shipping"
            class="dark:text-primary-400 text-primary text-xs">{{ shipping }}</span>
        </div>
      </div>
    </UCard>
  </NuxtLink>
